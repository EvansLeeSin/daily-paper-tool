...
export async function addWorkItem(workDate: string, content: string, source: 'git' | 'manual' = 'manual') {
  const db = await getDb();
...
export async function replaceWorkItems(workDate: string, items: Array<{ content: string; source?: 'git' | 'manual' }>) {
  const db = await getDb();
